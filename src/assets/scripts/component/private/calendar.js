...
var _CAL = {
    ...
    */  
    fixEvent: function(e) {
        ...
        e.preventDefaultEvent = function() {
            return e.preventDefault ? e.preventDefault : function(){e.returnValue = !1};    
        };
        e.stopPropagationEvent = function() {
            return e.stopPropagation ? e.stopPropagation : function(){e.cancelBubble = !0}; 
        };
        e.hoat = function() {
...
Calendar.prototype = {
    ...
    _init: function(config) {
        ...
        this.inputWidth = typeof config.inputWidth === 'number' ? config.inputWidth : null;
        this.inputHeight = typeof config.inputHeight === 'number' ? config.inputHeight : null;

        this.zIndex = typeof config.zIndex === 'number' ? config.zIndex : 1000;
        this.position = typeof config.position === 'string' ? config.position : "absolute";
        ...
        this.className = 'JS-calendar-wrapper';
        /**
         * 日历容器
         ...
         * @type Boolean
         */
        this.isShowWeek = typeof config.isShowWeek === 'boolean' ? config.isShowWeek : true;//add by huanming
    ...
    _create: function() {
        ...
        this.isPopup && (this.hide().container.style.position = this.position) && (this.container.style.zIndex = this.zIndex);//huanming z-index fixed
        ...
        if(this.triggerNode.tagName.toUpperCase() === "INPUT") {
            ...
            if(this.isDateInfo) {               
                this.triggerNodeParent                = document.createElement("span");//repaired by huanming (div --> span)
                ...
                this.triggerNodeParent.style.verticalAlign  = "middle";//add code by huanming

                if(typeof this.inputWidth === 'number'){
                    this.triggerNodeParent.style.width = this.inputWidth + "px";
                }else{
                    this.triggerNodeParent.style.width = this.triggerNode.offsetWidth + "px";
                }
                if(typeof this.inputHeight === 'number'){
                    this.triggerNodeParent.style.height = this.inputHeight + "px";
                }else{
            ...
            //如果不需要显示DateInfo
            if(!this.isShowWeek){
                this.oDateInfo.style.textIndent = '-999em';
            }
            //如果输入框有值
    ...
    _addEvent: function() {
        ...
        if(this.isPopup) {
            ...
            _CAL.on(window, "scroll", function() {
                that._setPos()  
            })
        }
        //给日期信息容器添加事件
    ...
    setDateInfo: function(sDate) {
        ...
        this.oDateInfo.innerHTML = this.rDate.test(sDate) || sDate === '' ? (this.triggerNode.value = sDate, this.render(sDate), this.getDateInfo(sDate)[this.isHoliday ? "holiday" : "week"]) : ""
    },
    /**
     * 获取指定间隔日期
